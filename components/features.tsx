  ...
  return (
    <>
      ...
      <Container className="w-[78rem] max-w-[90%] text-center">
        <p className="mx-auto my-16 text-2xl leading-tight text-blackcolor md:w-[80%] md:text-4xl">
          {text}
        </p>
...
const FeatureCards = ({ features }: FeatureCardsProps) => {
  return (
    <Container>
      <div className="grid w-full grid-cols-1 gap-6 md:grid-cols-2 text-blackcolor">
        {features.map(({ title, text, image, imageClassName }) => (
          ...
          >
            <h3 className="mb-2 text-2xl text-black">{title}</h3>
            <p className="max-w-[31rem] text-md text-blackcolor">{text}</p>
            <img
